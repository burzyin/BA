...
public class NearPlaceActivity extends BaseActivity {
    ...
    private List<Place> getPlaces() {
        Cursor c = getContentResolver().query(PlaceContentProvider.PLACES_URI, null, null, null, null);
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    File destinationDir = new File(Environment.getExternalStorageDirectory().getAbsolutePath()
                            + "/osmdroid");
                    if (!destinationDir.exists()) {
                        destinationDir.mkdirs();
                    }
    ...
    public void unzip() {
        ...
        if (!cacheFolder.exists())
            cacheFolder.mkdirs();
        try {
            ZipInputStream zipInputStream = new ZipInputStream(getAssets().open(
                    "minsk2.zip"));
            ZipEntry zipEntry;
            while ((zipEntry = zipInputStream.getNextEntry()) != null) {
                ...
                //else{
                FileOutputStream fileOutputStream = new FileOutputStream(unzipLocation
                        + zipEntry.getName());
    ...
    protected int getLayout() {
        return R.layout.near_list_layout;
    }
}